---
name: cosmos
description: Conventions for Cosmos ecosystem work — Cosmos SDK modules, CometBFT, IBC, protobuf, genesis, governance, chain upgrades, validator operations, plus gno.land and AtomOne. Use for any x/ module, app.go, .proto for a chain, genesis.json, config.toml/app.toml, or node/validator operations.
---

# Cosmos

Application-specific blockchains on the Cosmos stack: consensus, state machines,
IBC, and the full chain lifecycle from module development to mainnet operations.
Validator-operations rigor applies throughout: key management and upgrades are
infrastructure problems first, code problems second.
...
- **Cosmos SDK**: module architecture — keepers, msg servers, query servers,
  genesis, params, hooks, ABCI. Patterns follow the exact SDK version detected.
- **CometBFT**: BFT consensus, block production, validator sets, evidence
  handling, mempool behaviour, P2P, node configuration, consensus diagnostics.
- **IBC**: channels, ports, packet handling, acknowledgements, timeouts, relayer
  configuration. ICS-20 (transfer), ICS-27 (interchain accounts), ICS-721 (NFT).
- **Protobuf**: Cosmos-style messages, services and queries. `cosmos.msg.v1.signer`,
  amino annotations, codegen with `buf`.
- **Node operations**: state sync, snapshots, pruning, indexing, upgrades,
  validator management.

## Ecosystem: gno.land and AtomOne

Two Cosmos-adjacent stacks are in regular use. Detect which one before applying
generic Cosmos SDK patterns, because neither is a stock SDK chain.

### gno.land

- Gno is a deterministic Go interpreter, not the Go toolchain. `gnovm` executes
  Gno source directly; there is no compile step and no `go.mod` inside a realm.
- Realms (`r/`) are stateful and persist their globals between calls. Pure
  packages (`p/`) are stateless libraries. Keep business logic in `p/`, keep the
  persisted state and the exported entrypoints in `r/`.
- State persistence is automatic: assigning to a package-level var in a realm
  writes to the store. There is no explicit keeper or KVStore.
- `std.PreviousRealm()` / `std.OriginCaller()` are the authorization primitives.
  Never authorize on `std.OriginCaller()` where a contract may call on a user's
  behalf.
- Rendering: a realm's `Render(path string) string` returns markdown, which is
  what gnoweb serves. Treat it as the read API.
- Chain side is still Cosmos SDK + CometBFT (`gnoland`), so node ops, genesis and
  upgrades follow the normal rules below.

### AtomOne

- A governance-minimal Cosmos SDK fork of the Hub. The differences that matter
  are in the governance and staking parameters, not the module architecture.
- Constitution-gated governance: proposals are checked against an on-chain
  constitution. Read the current parameters from chain state before asserting how
  a vote resolves; do not assume Hub defaults.
- ATONE / PHOTON two-token model: fees are paid in PHOTON, staking is in ATONE.
  Never assume the staking denom is the fee denom.

### Validator operations

- Consensus keys never leave the signer. Use a remote signer (horcrux for
  threshold signing) rather than a plaintext `priv_validator_key.json` on the node.
- Double-signing is unrecoverable. Before any migration or restore, prove the old
  node is stopped and cannot come back; carry `priv_validator_state.json` forward,
  never regenerate it.
- Upgrades: run cosmovisor with `UNSAFE_SKIP_BACKUP=false`, pre-stage the upgrade
  binary, and confirm the binary hash against the governance proposal before the
  upgrade height.
- Keep state-sync snapshots and a recent archive restore path tested. An untested
